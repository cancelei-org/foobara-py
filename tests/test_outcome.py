...
class TestFailure:
    def test_is_failure(self):
        error = DataError(symbol="test", path=[], message="test error")
    ...
    def test_error_messages(self):
        error1 = DataError(symbol="e1", path=[], message="Error 1")
        error2 = DataError(symbol="e2", path=[], message="Error 2")
    ...
    def test_first_error(self):
        error = DataError(symbol="first", path=[], message="First error")
        outcome = Failure(errors=[error])
...
class TestCommandOutcome:
    ...
    def test_failure_from_errors(self):
        error = DataError(symbol="test", path=["field"], message="Invalid")
    ...
    def test_unwrap_failure_raises(self):
        error = DataError(symbol="test", path=[], message="Error")
        outcome = CommandOutcome.from_errors(error)
        with pytest.raises(ValueError):
            outcome.unwrap()
